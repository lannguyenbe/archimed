...
public class ContainerAdapter extends AbstractAdapter
{
    ...
    protected void renderDescriptiveSection() throws WingException, SAXException, CrosswalkException, IOException, SQLException 
    {
    	...
    	if (dso.getType() == Constants.COLLECTION) {
        	renderCollectionDescriptiveSection();
        	return;
        }
    ...
    // Lan 26.10.2016 : copy from ItemAdapter
    protected void renderCollectionDescriptiveSection() throws WingException, SAXException, CrosswalkException, IOException, SQLException
    {
        AttributeMap attributes;
        String groupID = getGenericID("group_dmd_");
        dmdSecIDS = new StringBuffer();
        ...
        if(dmdTypes.size() == 0 || dmdTypes.contains("DIM"))
        {
            ...
            startElement(DIM,"dim",attributes);
                ...
                Metadatum[] dcvs = dso.getMetadata(Item.ANY, Item.ANY, Item.ANY, Item.ANY);
                for (Metadatum dcv : dcvs)
                {
                        if (!MetadataExposure.isHidden(dspaceContext, dcv.schema, dcv.element, dcv.qualifier))
                        {
                        // ///////////////////////////////
