 ...
 * @author AdÃ¡n RomÃ¡n Ruiz <aroman@arvo.es> (Bugfix)
 */
...
//        query.addPara(null, "button-list").addButton("submit").setValue(T_go);
    ...
    // Lan 05.10.2016
    protected String getGroupBy() throws UIException {
        Request request = ObjectModelHelper.getRequest(objectModel);
        String group_by = decodeFromURL(request.getParameter("group_by"));
        if (group_by == null)
        {
            return "";
        }
        return group_by.trim();
    }
