...
public abstract class AbstractSearch extends AbstractDSpaceTransformer implements CacheableProcessingComponent{
        ...
        if (queryResults != null && 0 < queryResults.getDspaceObjects().size())
        {
            ...
            if(CollectionUtils.isNotEmpty(itemList))
            {
                ...
                for (Item resultDso : itemList)
                {
                    ...
                    renderItem(itemWingList, resultDso, highlightedResults, expandDocuments);
                }
            }
     ...
     * @param expandDocuments 
     * @throws WingException
     * @throws SQLException Database failure in services this calls
     */
    protected void renderItem(org.dspace.app.xmlui.wing.element.List dspaceObjectsList, Item item, DiscoverResult.DSpaceObjectHighlightResult highlightedResults, List<SearchDocument> expandDocuments) throws WingException, SQLException {
        ...
        //Lan 12.04.2015
        //Check our expand documents, we may need to add non-metadata "expanded" (from solr not from db)
        if(expandDocuments.size() > 0)
        {
            StringBuilder itemName = new StringBuilder();
            itemName.append(item.getHandle()).append(":").append("expanded");
            org.dspace.app.xmlui.wing.element.List expandList = itemList.addList(itemName.toString());
            for (SearchDocument docE : expandDocuments)
            {
                String value = docE.getSearchFieldValues("handle").get(0);
                addMetadataField(expandList, value);
            }
